...
- 雙詠唱模式:無詠唱(mueishō,直接喊法術名) / 詠唱(eishō,須先念呪文前置句)。
...
- **雙詠唱模式**(設定可切,沿用日系魔法術語):
  - **無詠唱(mueishō)** — 內部 id `mueisho`:辨識文字含法術別名即施法(例:「火球術」)。象徵高階法師,反應快,但聊天說話較容易誤觸發。
  - **詠唱(eishō)** — 內部 id `eisho`:須先念出**呪文(jumon)**前置句,再接法術名才發動。中二氣氛 max,誤觸發少。
    - **呪文(jumon)**:每個咏唱模式施法前要念的固定詠唱開頭句,作為發動的「鑰匙」。MVP 用單一通用呪文(暫定:「我命汝顯現」),比對時要求辨識文字在法術別名前出現該呪文(允許別名緊接在後,容許少量字差)。
    - 呪文字串集中定義在 `sim/spells.ts`(或 `voice/matcher.ts` 設定),可調整;日後可擴充成「每個法術專屬呪文」而不改架構。
  - 顯示給玩家的字用「無詠唱 / 詠唱」,程式內部 id 用英文(`mueisho` / `eisho`)。
...
- **語音比對單元測試**:正規化、中英別名命中、容錯(編輯距離)、無詠唱/詠唱雙模式、呪文前置句判定、誤觸發(不該命中的句子)。
