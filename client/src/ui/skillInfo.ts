...
export const SKILL_INFO: Record<SpellId, SkillInfo> = {
  ...
  frost:     { name: '冰結',            effect: '三連冰錐,命中減速',     stats: `傷害 ${CONFIG.frost.damage}×${CONFIG.frost.count} · 冷卻 ${SPELLS.frost.cooldown}s · 減速 ${CONFIG.frost.slowDuration}s`, detail: `朝滑鼠方向同時射出 ${CONFIG.frost.count} 道冰錐(扇形散射),命中造成傷害並大幅減速敵人,適合風箏與壓制推進。` },
  frostnova: { name: '冰霜新星',        effect: '自身周圍冰爆 + 減速',    stats: `傷害 ${CONFIG.frostnova.damage} · 冷卻 ${SPELLS.frostnova.cooldown}s · 範圍 ${CONFIG.frostnova.radius}`, detail: '以自身為中心瞬間引爆冰霜新星,對周圍所有敵人造成傷害並減速。被包圍時的解圍與保命技。' },
