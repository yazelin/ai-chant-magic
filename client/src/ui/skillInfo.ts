...
export const SKILL_INFO: Record<SpellId, SkillInfo> = {
  ...
  firestorm: { name: '爆裂魔法',        effect: '超大範圍爆裂(隨充能放大)', stats: `傷害 ${CONFIG.firestorm.baseDamage}+${CONFIG.firestorm.perChargeDamage}×層 · 冷卻 ${CONFIG.firestorm.baseCd}+層 s · 範圍 ${CONFIG.firestorm.baseRadius}+${CONFIG.firestorm.perChargeRadius}×層`, detail: `惠惠的招牌一擊。需先用詠唱疊「爆裂充能」,至少 1 層才放得出來;充能愈高愈猛:傷害 = ${CONFIG.firestorm.baseDamage} + 層數×${CONFIG.firestorm.perChargeDamage}、範圍隨層數放大(無上限)、冷卻 = ${CONFIG.firestorm.baseCd} + 層數 秒。放完清空所有充能。` },
  ...
  chain:     { name: '落雷',            effect: '連鎖閃電,最多跳 4 個',  stats: `傷害 ${CONFIG.chain.damage} · 冷卻 ${SPELLS.chain.cooldown}s · 跳躍 ${CONFIG.chain.maxJumps}`, detail: `落雷自最近的敵人開始連鎖,最多跳 ${CONFIG.chain.maxJumps} 個目標,每跳傷害遞減為 ${Math.round(CONFIG.chain.falloff * 100)}%。清成群雜兵的神技。` },
  ...
  aegis:     { name: '聖盾',            effect: '全隊護盾 + 技能/治療強化',         stats: `持續 ${CONFIG.aegis.duration}s · 範圍 ${CONFIG.aegis.radius} · 傷害/治療 ×2 · 冷卻 ${SPELLS.aegis.cooldown}s`, detail: '揮舞聖旗,為範圍內所有隊友(含自己)張開聖盾。聖盾存在期間,全隊所有技能造成的傷害變為 2 倍,治療技能的恢復量也變為 2 倍;聖盾消失後立刻恢復正常。' },
